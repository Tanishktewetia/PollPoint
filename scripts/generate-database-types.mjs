...
try {
  const { rows: columns } =
    await db.query(`select table_name, column_name, udt_name, is_nullable, column_default
  ...
  function type(name) {
    if (
      ["uuid", "text", "timestamptz", "timestamp", "date", "varchar"].includes(
        name,
      )
    )
      return "string";
    if (["int2", "int4", "int8", "float4", "float8", "numeric"].includes(name))
      return "number";
  ...
  const tables = [...new Set(columns.map((c) => c.table_name))]
    .map((table) => {
      ...
      const shapes = ["Row", "Insert", "Update"]
        .map(
          (shape) =>
            `${shape}: {\n${fields
              .map(
                (c) =>
                  `        ${c.column_name}${shape === "Update" || (shape === "Insert" && (c.column_default !== null || c.is_nullable === "YES")) ? "?" : ""}: ${type(c.udt_name)}${c.is_nullable === "YES" ? " | null" : ""};`,
              )
              .join("\n")}\n      };`,
        )
        .join("\n      ");
  ...
  const rpc = functions
    .map(
      (f) =>
        `    ${f.proname}: { Args: ${f.argtypes.length ? `{ ${f.argtypes.map((a, i) => `${f.proargnames[i]}: ${type(a)}${f.proname === "admin_save_survey" && i < 2 ? " | null" : ""}`).join("; ")} }` : "Record<PropertyKey, never>"}; Returns: ${type(f.result_type)} };`,
    )
    .join("\n");
  ...
  console.log(
    `Generated types for ${new Set(columns.map((c) => c.table_name)).size} tables and ${functions.length} functions from clean migration replay.`,
  );
} finally {
  await db.close();
}
